...
def trainNeuralNetworkSupplyChain(
    ...
    NetLogoDataFitterNN(
        ...
        parametersIn,
        parametersOut,
        modelContructor(len(parametersIn), len(parametersOut), hiddenLayers=4),
        tag=tag_full,
...
def create_mzn_by_tag(tag, folder, parametersIn, paramsOut):
    torch.load(folder + NetLogoDataFitterNN.FILENAME_PARAM_OUT + tag + ".pt").createMiniZincFunctions(
        parametersIn,
        paramsOut,
        os.path.join(folder, f"nn_2layer_{tag}.mzn"),
        tag=create_full_tag(tag)
    )
...
if __name__ == "__main__":
    ...
    trainNeuralNetworkSupplyChain(
        folder, tag + "_pow2", max_seconds=max_seconds, modelContructor=LinearNetPow2,
        testDataPath=testDataPath, printTestLabels=printTestLabels
    )
    # print("sigmoid")
    # trainNeuralNetworkSupplyChain(
    #     folder, tag+"_sigmoid", max_seconds=max_seconds, modelContructor=LinearNetSigmoid,
    #     testDataPath=testDataPath, printTestLabels=printTestLabels
    # )
    # print("LinearDoubleRelu")
    # trainNeuralNetworkSupplyChain(
    #     folder, tag+"_doubleRelu", max_seconds=max_seconds, modelContructor=LinearDoubleRelu,
    #     testDataPath=testDataPath, printTestLabels=printTestLabels
    # )
    # print("taylor")
    # trainNeuralNetworkSupplyChain(
    #     folder, tag+"_taylor", max_seconds=max_seconds, modelContructor=LinearNetTaylorSimple,
    #     testDataPath=testDataPath, printTestLabels=printTestLabels
    # )
    # create_mzn_by_tag(create_full_tag(tag), folder, get_parameters_in(), get_parameters_out())
