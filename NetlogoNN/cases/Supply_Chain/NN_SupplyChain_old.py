...
# from NetLogoSimulator import NetLogoSimulatorUniform
...
if __name__ == "__main__":
    ...
    hiddenLayers = 3
    tag = f"Supply_Chain_{hiddenLayers}"
    ...
    folder = "data_out/Supply_Chain_halton_v3/"
    ...
    NetLogoDataFitterNN(
        ...
        parametersIn,
        parametersOut,
        LinearNetSigmoid(len(parametersIn), len(parametersOut), hiddenLayers=hiddenLayers),
        tag=tag,
    ...
    def create_mzn_by_tag(tag, paramsOut):
        torch.load(folder + NetLogoDataFitterNN.FILENAME_PARAM_OUT + tag + ".pt").createMiniZincFunctions(
            parametersIn,
            paramsOut,
            f"mzn_nn/nn_out_sigmoid_{hiddenLayers}layer.mzn", tag=tag
        )
